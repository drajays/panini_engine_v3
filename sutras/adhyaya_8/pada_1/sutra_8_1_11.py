"""
8.1.11  कर्मधारयवत् उत्तरेषु  —  VIDHI

Padaccheda: कर्मधारय-वत् उत्तरेषु

कर्मधारयवत् उत्तरेषु (8.1.11)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import tripadi_gate_eligible

_GATE_KEY: str = "8_1_11_karmaDAray_11"


def cond(state: State) -> bool:
    return tripadi_gate_eligible(state, "8.1.11", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["sandhi_kind"]             = "8.1.11"
    return state


SUTRA = SutraRecord(
    sutra_id              = "8.1.11",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "karmaDArayavat uttarezu",
    text_dev              = "कर्मधारयवत् उत्तरेषु",
    padaccheda_dev        = "कर्मधारय-वत् उत्तरेषु",
    why_dev               = "(सूत्रम् 8.1.11) कर्मधारयवत् उत्तरेषु।",
    anuvritti_from        = ('8.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
