"""
6.1.130  ई३ चाक्रवर्मणस्य  —  VIDHI

Padaccheda: ई३ (लुप्तप्रथमान्तनिर्देशः) चाक्रवर्मणस्य

ई३ चाक्रवर्मणस्य (6.1.130)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_1_130_I3_130"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.1.130", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.1.130"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.1.130",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "I3 cAkravarmaRasya",
    text_dev              = "ई३ चाक्रवर्मणस्य",
    padaccheda_dev        = "ई३ (लुप्तप्रथमान्तनिर्देशः) चाक्रवर्मणस्य",
    why_dev               = "(सूत्रम् 6.1.130) ई३ चाक्रवर्मणस्य।",
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
