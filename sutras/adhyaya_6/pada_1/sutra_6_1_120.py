"""
6.1.120  अनुदात्ते च कुधपरे  —  VIDHI

Padaccheda: अनुदात्ते च कु-ध-परे

अनुदात्ते च कुधपरे (6.1.120)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_1_120_anudAtte_120"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.1.120", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.1.120"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.1.120",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "anudAtte ca kuDapare",
    text_dev              = "अनुदात्ते च कुधपरे",
    padaccheda_dev        = "अनुदात्ते च कु-ध-परे",
    why_dev               = "(सूत्रम् 6.1.120) अनुदात्ते च कुधपरे।",
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
