"""
6.1.109  एङः पदान्तादति  —  VIDHI

Padaccheda: एङः पदान्तात् अति

एङः पदान्तादति (6.1.109)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_1_109_eNaH_109"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.1.109", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.1.109"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.1.109",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "eNaH padAntAdati",
    text_dev              = "एङः पदान्तादति",
    padaccheda_dev        = "एङः पदान्तात् अति",
    why_dev               = "(सूत्रम् 6.1.109) एङः पदान्तादति।",
    apavada_of     = ("6.1.78",),   # एङः पदान्तादति is the अपवाद of एचोऽयवायावः: pada-final eṅ before short a gives pūrvarūpa.
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
