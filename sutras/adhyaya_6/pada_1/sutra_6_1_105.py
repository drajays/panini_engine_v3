"""
6.1.105  दीर्घाज्जसि च  —  VIDHI

Padaccheda: दीर्घात् जसि च

दीर्घाज्जसि च (6.1.105)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_1_105_dIrGAjjasi_105"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.1.105", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.1.105"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.1.105",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "dIrGAjjasi ca",
    text_dev              = "दीर्घाज्जसि च",
    padaccheda_dev        = "दीर्घात् जसि च",
    why_dev               = "(सूत्रम् 6.1.105) दीर्घाज्जसि च।",
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
