"""
6.1.7  तुजादीनां दीर्घोऽभ्यासस्य  —  VIDHI

Padaccheda: तुज्-आदीनाम् दीर्घः अभ्यासस्य

तुजादीनां दीर्घोऽभ्यासस्य (6.1.7)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_1_7_tujAdInAM_7"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.1.7", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.1.7"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.1.7",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "tujAdInAM dIrGo'ByAsasya",
    text_dev              = "तुजादीनां दीर्घोऽभ्यासस्य",
    padaccheda_dev        = "तुज्-आदीनाम् दीर्घः अभ्यासस्य",
    why_dev               = "(सूत्रम् 6.1.7) तुजादीनां दीर्घोऽभ्यासस्य।",
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
