"""
6.2.198  सक्थं चाक्रान्तात्  —  VIDHI

Padaccheda: सक्थम् च अक्रान्तात्

सक्थं चाक्रान्तात् (6.2.198)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_2_198_sakTaM_198"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.2.198", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.2.198"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.2.198",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "sakTaM cAkrAntAt",
    text_dev              = "सक्थं चाक्रान्तात्",
    padaccheda_dev        = "सक्थम् च अक्रान्तात्",
    why_dev               = "(सूत्रम् 6.2.198) सक्थं चाक्रान्तात्।",
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
