"""
6.2.71  भक्ताख्यास्तदर्थेषु  —  VIDHI

Padaccheda: भक्ताख्याः तदर्थेषु

भक्ताख्यास्तदर्थेषु (6.2.71)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_2_71_BaktAKyAst_71"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.2.71", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.2.71"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.2.71",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "BaktAKyAstadarTezu",
    text_dev              = "भक्ताख्यास्तदर्थेषु",
    padaccheda_dev        = "भक्ताख्याः तदर्थेषु",
    why_dev               = "(सूत्रम् 6.2.71) भक्ताख्यास्तदर्थेषु।",
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
