"""
6.4.135  षपूर्वहन्धृतराज्ञामणि  —  VIDHI

Padaccheda: ष-पूर्व-हन्-धृतराज्ञाम् अणि

षपूर्वहन्धृतराज्ञामणि (6.4.135)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.gates import adhikara_in_effect
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_4_135_zapUrvahan_135"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.4.135", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.4.135"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.4.135",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "zapUrvahanDftarAjYAmaRi",
    text_dev              = "षपूर्वहन्धृतराज्ञामणि",
    padaccheda_dev        = "ष-पूर्व-हन्-धृतराज्ञाम् अणि",
    why_dev               = "(सूत्रम् 6.4.135) षपूर्वहन्धृतराज्ञामणि।",
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
