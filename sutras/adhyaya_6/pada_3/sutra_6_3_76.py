"""
6.3.76  एकादिश्चैकस्य चादुक्  —  VIDHI

Padaccheda: एक-आदिः च एकस्य च आदुक्

एकादिश्चैकस्य चादुक् (6.3.76)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import samhita_gate_eligible

_GATE_KEY: str = "6_3_76_ekAdiScEka_76"


def cond(state: State) -> bool:
    return samhita_gate_eligible(state, "6.3.76", gate_key=_GATE_KEY)


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["anga_kind"]             = "6.3.76"
    return state


SUTRA = SutraRecord(
    sutra_id              = "6.3.76",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "ekAdiScEkasya cAduk",
    text_dev              = "एकादिश्चैकस्य चादुक्",
    padaccheda_dev        = "एक-आदिः च एकस्य च आदुक्",
    why_dev               = "(सूत्रम् 6.3.76) एकादिश्चैकस्य चादुक्।",
    anuvritti_from        = ('6.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
