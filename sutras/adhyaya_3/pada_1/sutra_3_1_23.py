"""
3.1.23  नित्यं कौटिल्ये गतौ  —  VIDHI

Padaccheda: नित्यम् कौटिल्ये गतौ

Krt suffix rule from dhatu: नित्यं कौटिल्ये गतौ (23)
"""
from __future__ import annotations

from engine       import SutraType, SutraRecord, register_sutra
from engine.state import State
from engine.krt_eligibility import krt_insertion_eligible

_GATE_KEY: str = "3_1_23_nityaM_23"


def cond(state: State) -> bool:
    if not krt_insertion_eligible(state, "3.1.23", gate_key=_GATE_KEY, adhikara_id="3.1.1"):
        return False
    return not any(
        "krt" in t.tags and "pratyaya" in t.tags for t in state.terms
    )


def act(state: State) -> State:
    state.paribhasha_gates[_GATE_KEY] = True
    state.samjna_registry[_GATE_KEY]  = True
    state.meta["krt_kind"] = "3.1.23"
    return state


SUTRA = SutraRecord(
    sutra_id              = "3.1.23",
    sutra_type            = SutraType.VIDHI,
    r1_form_identity_exempt = True,
    text_slp1             = "nityaM kOwilye gatO",
    text_dev              = "नित्यं कौटिल्ये गतौ",
    padaccheda_dev        = "नित्यम् कौटिल्ये गतौ",
    why_dev               = "धातोः [नित्यं कौटिल्ये गतौ]-प्रत्ययः विहितः (३.१.23)।",
    anuvritti_from        = ('3.1.1',),
    cond                  = cond,
    act                   = act,
)

register_sutra(SUTRA)
