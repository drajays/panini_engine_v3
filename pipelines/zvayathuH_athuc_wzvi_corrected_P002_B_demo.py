"""
pipelines/zvayathuH_athuc_wzvi_corrected_P002_B_demo.py — **P002-B** श्वयथुः.

Source row **P002-B** in ``corrected_prakriyas_v2``: *ṭuoñśvi* (human टुओँश्वि;
machine upadeśa ``wzvi`` — ṭu *it*, stem ``zvi``) + *athuc* (**3.3.89**) →
*śvayathuḥ*.

Spine: ñi-*it* … ``zvi`` → **3.1.91** *adhikāra* → **3.3.89** *athuc* → *it*-*lopa*
→ **7.3.84** / **6.1.78** (``zve`` + ``athu`` → ``zvayathu``) → ``_pada_merge``;
**1.2.46** Case L; **4.1.1** / **4.1.2** + Tripāḍī tail.

SLP1 surface: ``zvayathuH`` (``z`` = तालव्य श्).
...
def derive_zvayathuH_athuc_wzvi_corrected_P002_B() -> State:
    dhatu = Term(
        kind="prakriti",
    ...
    s.meta["corrected_v2_P002_B_3_3_89_arm"] = True
...
__all__ = ["derive_zvayathuH_athuc_wzvi_corrected_P002_B", "_UPADESHA"]
